...
class SpectralConvAutoencoder_4L12DW(nn.Module):
    def __init__(self, latent_dim=100):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Conv2d(1, 10, kernel_size=1),  # pointwise
            nn.GroupNorm(5, 10), # 5 groups for 10 channels
            nn.ReLU(),

            # Depthwise + pointwise conv2: (12 → 20)
            nn.Conv2d(10, 10, kernel_size=3, padding=1, groups=10),  # depthwise
            nn.Conv2d(10, 16, kernel_size=1),  # pointwise
            nn.GroupNorm(4, 16),  # 4 groups for 16 channels
            nn.ReLU(),

            nn.Dropout(p=0.1),

            nn.Conv2d(16, 16, kernel_size=3, stride=2, padding=1, groups=16),  # depthwise
            nn.Conv2d(16, 20, kernel_size=1),  # pointwise
            nn.GroupNorm(4, 20),
            nn.ReLU(),
